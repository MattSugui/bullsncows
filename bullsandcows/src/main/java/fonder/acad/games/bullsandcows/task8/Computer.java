package fonder.acad.games.bullsandcows.task8;


import fonder.acad.games.bullsandcows.IntegerPair;
import java.util.*;
import java.io.*;
import java.time.*;
import java.time.temporal.*;

/**
 * The base computer user.
 * @author Adrian
 */
public class Computer
{
    public Computer(Scanner scan, boolean think)
    {
        this.randomiser = new Random(); if (think) this.code = think(); 
        this.scan = scan; this.entries = new ArrayList<>();
    }
    public Computer(Scanner scan, String[] prem, boolean think)
    {
        this.randomiser = new Random(); if (think) this.code = think();
        this.scan = scan; this.premoves = prem;
        this.entries = new ArrayList<>();
    }
    
    protected Random randomiser;
    protected String code;
    protected Scanner scan;
    protected int index;
    protected String[] premoves;
    public ArrayList<String> entries;
    
    public String getCode() { return this.code; } 
    
    public Scanner getScanner() { return this.scan; } public void setScanner(Scanner scan) { this.scan = scan; }
    
    public static boolean verify(String seq)
    {
        for (int i = 0; i < 4; i++) for (int j = 0; j < 4; j++)
            if (seq.charAt(i) == seq.charAt(j) && i != j) return false;
        return true;
    }
 
    public String respond()
    {
        if (premoves != null && index < premoves.length)
        {
            System.out.println("Reading from premove queue: "+premoves[index]);
            return premoves[index++];
        }
        return prompt(false);
    }
    
    public String prompt(boolean first)
    {
        String cod;
        while (true)
        {
            System.out.print("Enter your guess: ");
            cod = scan.next();
            try { int _ = Integer.parseInt(cod); } catch (NumberFormatException _) { System.out.println("\n? code must be purely numerical"); continue; }
            if (cod.length() != 4) { System.out.println("\n? code must have four digits"); continue; }
            else if (!verify(cod)) { System.out.println("\n? code must not have repeating digits"); continue; }
            
            return cod;
        }      
    }
    
    public int invoke()
    {   
        System.out.println("Your turn!");
        String p = prompt(false);
        IntegerPair a = scan(p);
        System.out.printf("%d bull%s, %d cow%s\n", a.item1, a.item1 != 1? "s":"", a.item2, a.item2 != 1? "s":"");
        
        entries.add(String.format("You submitted %s, which revealed %s bull%s and %s cow%s.\n",
               p,
               a.item1 == 0? "no":Integer.toString(a.item1),
               a.item1 != 1? "s":"",
               a.item2 == 0? "no":Integer.toString(a.item2),
               a.item2 != 1? "s":""));
            
        
        if (a.item1 == 4) return 1;
        
        return 0;
    }
    
    public boolean print(String path)
    {
        try (FileWriter logger = new FileWriter(path))
        {
            logger.write(String.format("""
                                       Matt's Bulls and Cows Game!
                                       Created 2025. Released under the terms of the data privacy guidelines of the University of Santo Tomas (Do not redistribute!)
                                       
                                       Game results receipt
                                       Session no. %sw%d-%s
                                       
                                       The computer's secret code against you was %s.
                                       ==================================================
                                       """,
                        Integer.toString(LocalDate.now().getYear()).substring(1, 4),
                        LocalDate.now().get(WeekFields.of(Locale.getDefault()).weekOfWeekBasedYear()),
                        Integer.toHexString(LocalDate.now().hashCode()),
                        code));
            //Matt's Bulls and Cows Game!\nCreated 2025. CC-BY-SA-NC 4.0 (do not redistribute!)\n\nGame results receipt\n\n");
            for (String line: entries.toArray(String[]::new)) logger.write(line);
        }
        catch (IOException e)
        {
            System.out.printf("! error occurred whilst writing file: %s\n", e.getMessage());
            return false;
        }
        return true;
    }
    
    protected IntegerPair scan(String seq)
    {        
        int bulls = 0, cows = 0;
               
        for (int i = 0; i < 4; i++) for (int j = 0; j < 4; j++)
            if (seq.charAt(i) == this.code.charAt(j))
                if (i == j) bulls++; else cows++;
        
        return new IntegerPair(bulls, cows);
    }

    protected String think()
    {
        while (true)
        {
            String blip = String.format("%04d", randomiser.nextInt(0,9999));
            if (verify(blip)) return blip;
        }
    }
}